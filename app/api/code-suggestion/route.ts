...
import { generateInlineCompletion } from "@/lib/ai"
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Analyze code context (used for response metadata)
    const context = analyzeCodeContext(fileContent, cursorLine, cursorColumn, fileName)

    // Split the file at the cursor for fill-in-the-middle completion
    const { prefix, suffix } = splitAtCursor(fileContent, cursorLine, cursorColumn)

    // Generate the completion using the model's native FIM format
    const suggestion = await generateSuggestion(prefix, suffix)
 ...
 * cursorLine and cursorColumn are 0-based (as sent by the editor hook).
 */
function splitAtCursor(content: string, cursorLine: number, cursorColumn: number) {
  const lines = content.split("\n")
  const currentLine = lines[cursorLine] ?? ""
  const before = lines.slice(0, cursorLine)
  const after = lines.slice(cursorLine + 1)

  const prefix = [...before, currentLine.slice(0, cursorColumn)].join("\n")
  const suffix = [currentLine.slice(cursorColumn), ...after].join("\n")
  return { prefix, suffix }
}

/**
 * Generate a code completion using the configured AI provider (see lib/ai.ts).
 * Returns "" on failure so no ghost text is shown.
 */
async function generateSuggestion(prefix: string, suffix: string): Promise<string> {
  try {
    return await generateInlineCompletion(prefix, suffix)
  } catch (error) {
