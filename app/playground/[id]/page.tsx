...
import { CollabRoom } from "@/features/collaboration/components/collab-room";
...
const MainPlaygroundPage: React.FC = () => {
  ...
  // Monaco editor/monaco instances, exposed by PlaygroundEditor on mount.
  const aiEditorRef = useRef<any>(null);
  const aiMonacoRef = useRef<any>(null);

  const handleEditorMount = useCallback((editor: any, monaco: any) => {
    aiEditorRef.current = editor;
    aiMonacoRef.current = monaco;
  }, []);

  // Insert AI chat code into the editor. Replaces a selection if there is one,
  // otherwise inserts the block on its own line(s) so it never merges with
  // surrounding code (e.g. avoids `}import {...}` jamming together).
  const handleInsertAICode = useCallback((code: string) => {
    const editor = aiEditorRef.current;
    const monaco = aiMonacoRef.current;
    if (!editor || !monaco) {
    ...
    // 1) Explicit selection -> replace exactly what's selected.
    ...
    // 2) Full-file response (has imports / export default) -> replace the whole
    //    file so we don't stack a second copy on top of the existing code.
    const looksLikeFullFile =
      /^\s*import\s/m.test(code) || /export\s+default\b/.test(code);
    ...
    if (looksLikeFullFile) {
      ...
      editor.getAction("editor.action.formatDocument")?.run?.();
      editor.focus();
      toast.success("Replaced file with AI code");
      return;
    }

    // 3) Snippet -> insert at the cursor, padded with newlines so it stays on
    //    its own line(s) and doesn't fuse with adjacent code.
    ...
    if (beforeCursor.trim().length > 0) text = "\n" + text;
    if (afterCursor.trim().length > 0) text = text + "\n";
    ...
    editor.getAction("editor.action.formatDocument")?.run?.();
    editor.focus();
    toast.success("Inserted code into editor");
  }, []);
  ...
  React.useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      // Support both Ctrl+S (Windows/Linux) and Cmd+S (macOS)
                ...
                <div className="border-b bg-muted/30">
                  ...
                  >
                    <div className="flex items-center justify-between px-4 py-2 gap-2">
                      ...
                      <TabsList className="h-8 bg-transparent p-0 w-max">
                        {openFiles.map((file) => (
                          <TabsTrigger
                            key={file.id}
                            value={file.id}
                            className="relative h-8 px-3 shrink-0 whitespace-nowrap data-[state=active]:bg-background data-[state=active]:shadow-sm group"
                          >
