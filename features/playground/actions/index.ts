...
export const deleteProjectById = async (id:string)=>{
    ...
    try {
        ...
        const playground = await db.playground.findUnique({ where: { id }, select: { userId: true } });
        if (!playground || playground.userId !== user?.id) return;
...
export const editProjectById = async (id:string,data:{title:string , description:string})=>{
    const user = await currentUser();
    try {
        // Only the owner may edit project metadata.
        const playground = await db.playground.findUnique({ where: { id }, select: { userId: true } });
        if (!playground || playground.userId !== user?.id) return;
